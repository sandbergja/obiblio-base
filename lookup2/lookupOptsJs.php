...
oed = {
	...
	fetchOpts: function () {
	  ...
	  $.get(oed.url,{mode:'getOpts'}, function(response){
	  	if (response.indexOf('doesn&#039;t exist') >= 0) {
				alert ("'lookup_settings'Table does not exist!!")
				$.post(oed.url,{mode:'createOptsTbl'}, function (response) {
					if (response == '1') {
						alert ("A new, empty, 'lookup_settings' table has been created, please try again");
					} else {
						alert(response);
					}
					return false;
