...
hed = {
	...
	fetchHosts: function () {
	  ...
	  $.get(hed.url,{mode:'getHosts'}, function(response){
	  	if (response.indexOf('doesn&#039;t exist') >= 0) {
				alert ("'lookup_hosts'Table does not exist!!")
				$.post(hed.url,{mode:'createHostTbl'}, function (response) {
					if (response == '1') {
						alert ("A new, empty, 'lookup_hosts' table has been created, please try again");
					} else {
						alert(response);
					}
					return false;
	  		...
	  		hed.hostJSON = JSON.parse(response);
			var html = '';
    		...
    		html += '<td valign="top" class="primary">'+hed.hostJSON[nHost]['context']+'</td>\n';
    		html += '<td valign="top" class="primary">'+hed.hostJSON[nHost]['schema']+'</td>\n';
